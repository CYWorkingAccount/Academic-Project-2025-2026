...
Le lancement du programme en ligne de commande reste identique à la Version 2. La nouveauté majeure lors de l'exécution est la gestion d'un fichier de sauvegarde binaire nommé `historique.sae`. Ce fichier est créé automatiquement à la racine de l'application lors du premier enregistrement d'un voyage, et est rechargé dynamiquement aux lancements suivants pour restaurer l'historique de l'utilisateur.
...
image::UML version 3.png[Diagramme UML Version 3]

Pour répondre aux exigences de la Version 3 (sauvegarde de l'historique et préférences relatives), l'architecture objet a subi les évolutions suivantes :

* **Implémentation de `Serializable` :** Pour permettre la sauvegarde binaire des trajets, il a fallu rendre les objets sérialisables. Ainsi, les classes constituant un itinéraire (`Voyage`, `Trajet`, `Arret`, et `Cout`) implémentent toutes désormais l'interface standard Java `java.io.Serializable`.
* **Création de la classe `Historique` :** Cette nouvelle classe gère une liste (`ArrayList`) de `Voyage`s. Elle a la responsabilité d'encapsuler la logique de lecture et d'écriture dans le fichier binaire (flux d'entrée/sortie).
* **Évolution multicritère :** La logique de décision de l'utilisateur (initialement basée sur un critère unique dans `Voyageur`) a évolué pour prendre en compte une pondération (pourcentages) des différents coûts. 

=== 3. Analyse technique et critique de l'implémentation

* **Sérialisation binaire :** L'historique est sauvegardé à l'aide des flux `ObjectOutputStream` et `FileOutputStream`. Le chargement utilise `ObjectInputStream`. Nous avons géré les exceptions potentielles (ex: fichier inexistant au premier lancement, erreur de lecture) via des blocs `try-catch` afin que l'application ne crashe jamais et initialise simplement un historique vide si besoin.
* **Tri multi critère :** Pour gérer "l'importance relative" des critères voulue par la Version 3, l'algorithme trie par rapport au critère le plus important. Si deux valeurs sont les même, il trie alors par rapport au deuxième critère, puis au troisième, cela assure un tri favorisant toujours les critères dans l'ordre de l'utilisateur peu importe leurs valeurs.
* **Optimisation de l'affichage historique :** De nouvelles méthodes de formatage (`getVillesUniques()`, `toStringHistorique()`) ont été ajoutées pour extraire uniquement les villes de départ et d'arrivée, afin de fournir un résumé clair adapté à la vue historique de l'interface graphique.

=== 4. Analyse quantitative et qualitative des tests

La suite de tests unitaires a été complétée pour couvrir le cœur de la Version 3 :

* **Tests de la classe `HistoriqueTest` :** Nous avons mis en place des scénarios pour vérifier que la sauvegarde et le chargement des voyages fonctionnent sans perte de données. Cela inclut la vérification de l'intégrité du graphe d'objets après désérialisation (vérifier qu'un `Voyage` rechargé contient bien ses `Trajet`s et ses `Cout`s originaux).
* **Robustesse du multicritère :** Les méthodes de tri et de filtrage ont été testées pour s'assurer que le tri respecte bien l'ordre de priorité des critères, même dans des cas où les valeurs sont identiques pour certains critères.